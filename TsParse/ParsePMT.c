...
static void GetPMT_Info(TS_PMT *pstTS_PMT, int iStreamCount, PMT_INFO *pstPMT_Info, int *iVideoCount)
{
	...
	/*pcr*/
	pstPMT_Info->pcrPID = pstTS_PMT->PCR_PID;
	int pesCount = 0;
	for (iLoopTime = 0; iLoopTime < iStreamCount; iLoopTime++)
	{
		...
		/*----------------------------------------------------------*/
		/* get the pes private PID */
		/*----------------------------------------------------------*/
		if(0x06 == uiStreamType)
		{
			pstPMT_Info->pesPrivatePID[pesCount] = uiElementrayPID;
			pesCount++;
		//	printf("pesPrivatePID = %d\n",pstPMT_Info->pesPrivatePID[iLoopTime]);
		}
	}
}
/*********************************************************************************************************************************
