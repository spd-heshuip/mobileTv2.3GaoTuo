	...
	for (iLoopTime = 0; iLoopTime < iProgramCount; iLoopTime++)
	{
		for (iStreamIndex = 0; iStreamIndex < iProgramCount; iStreamIndex++)
		{
			if (pstProgramInfo[iLoopTime].uiProgramNumber == pstPMT_Info[iStreamIndex].uiProgramNumber)
			{
				...
				pstProgramInfo[iLoopTime].pcrPID = pstPMT_Info[iStreamIndex].pcrPID;	
				memcpy(pstProgramInfo[iLoopTime].pesPrivatePID,pstPMT_Info[iStreamIndex].pesPrivatePID,50 * (sizeof(unsigned int)));
			}
		}
	}
}
...
//		printf("------------pcr_pid = %d\n-----------",pstProgramInfo[iProgramIndex].pcrPID);
//		unsigned char pids[1024] = {0};
//		IntToString(pstProgramInfo[iProgramIndex].pesPrivatePID,pids,50);
//		printf("------------pes_pids = %s\n----------",pids);
	}
#endif
}
 ...
 *********************************************************************************************************************************/
PROGRAM_INFO GetProgram(PROGRAM_INFO *pstProgramInfo, int iProgramCount, unsigned int uiServiceId)
{
	int iProgramIndex = 0;
