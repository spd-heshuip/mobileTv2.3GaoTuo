...
static int usb_bulk_io(libusb_device_handle *handle, int ep, char *bytes,
...
API_EXPORTED int usb_bulk_read(libusb_device_handle *handle, int ep, char *bytes,
	int size, int timeout)
{
...
API_EXPORTED int usb_bulk_write(libusb_device_handle *handle, int ep, const char *bytes,
	int size, int timeout)
{
	if (ep & USB_ENDPOINT_IN) {
	...
	return usb_bulk_io(handle, ep, (char *)bytes, size, timeout);
}
...
API_EXPORTED int usb_interrupt_read(libusb_device_handle *handle, int ep, char *bytes,
	int size, int timeout)
{
	if (!(ep & USB_ENDPOINT_IN)) {
		/* libusb-0.1 will strangely fix up a read request from endpoint
...
API_EXPORTED int usb_interrupt_write(libusb_device_handle *handle, int ep, const char *bytes,
	int size, int timeout)
{
	if (ep & USB_ENDPOINT_IN) {
		/* libusb-0.1 on BSD strangely fix up a write request to endpoint
	...
	return usb_interrupt_io(handle, ep, (char *)bytes, size, timeout);
}

API_EXPORTED int usb_control_msg(libusb_device_handle *handle, int bmRequestType,
...
//		bRequest, wValue, wIndex, size, timeout);
	...
	r = libusb_control_transfer(handle, bmRequestType & 0xff,
