/*------------------------------------------------------------------------------
  ...
  Last Updated  : $Date:: 2012-07-23 16:22:09 #$
  File Revision : $Revision:: 5836 $
------------------------------------------------------------------------------*/
...
static sony_result_t ATtoSLT(sony_demod_t * pDemod)
{
    ...
    if(pDemod->sharedIf) {
        ...
        /* Disable ADC 2 */
        if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x43, 0x02) != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }

        /* Disable ADC 3 */
        if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x41, 0x02) != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }
    }
