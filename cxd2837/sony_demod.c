/*------------------------------------------------------------------------------
  ...
  Last Updated  : $Date:: 2012-11-13 12:09:09 #$
  File Revision : $Revision:: 6390 $
------------------------------------------------------------------------------*/
...
#endif
unsigned char TunMode=45,DemMode=37,TunerSMode=1,TunerHV=0;
...
//	printf("result XtoSLT=%d\n",result);
	...
	//printf("result sony_demod_ChipID=%d\n",result);
    if (result != SONY_RESULT_OK) {
	...
	//printf("result clearConfigMemory=%d\n",result);
    if (result != SONY_RESULT_OK) {
        pDemod->state = SONY_DEMOD_STATE_INVALID;
        SONY_TRACE_RETURN (result);
    }
                                    ...
                                    int32_t value)
{
    ...
    case SONY_DEMOD_CONFIG_SAT_TUNER_IQ_SENSE_INV:
        ...
        case 1:
            /* Inverted */
            pDemod->satTunerIqSense = SONY_DEMOD_SAT_IQ_SENSE_INV;
            break;
    ...
    case SONY_DEMOD_CONFIG_SAT_IFAGCNEG:
        ...
        /* ORFAGC_AGCNEG */
        result = sony_demod_SetAndSaveRegisterBits(pDemod, pDemod->i2cAddressSLVT, 0xA0, 0xB9, (uint8_t)value, 0xFF);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
...
static sony_result_t XtoSLT(sony_demod_t * pDemod)
{
...

	{
		...
		if(TunMode==20){
			...
			sony_demod_GPIOSetConfig (pDemod, 1, 1, 1);
			sony_demod_GPIORead (pDemod, 1,&Value);
			if(Value)
				pDemod->xtalFreq = SONY_DEMOD_XTAL_41000KHz;
			else
				pDemod->xtalFreq = SONY_DEMOD_XTAL_16000KHz;

			//printf("pDemod->xtalFreq;%d \n",pDemod->xtalFreq);

		}
		if((TunMode==35)||(TunMode==38)||(TunMode==41)||(TunMode==46))
		{
			uint8_t Value;
			sony_demod_xtal_t xtalFreq = SONY_DEMOD_XTAL_41000KHz;
			sony_demod_GPIOSetConfig (pDemod, 1, 1, 1);
