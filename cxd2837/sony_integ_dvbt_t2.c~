/*------------------------------------------------------------------------------
  ...
  Last Updated  : $Date:: 2012-07-24 16:34:42 #$
  File Revision : $Revision:: 5850 $
------------------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------------
 Functions
------------------------------------------------------------------------------*/
extern unsigned char TunMode,TunerHV;
...
//ning 
if(((TunMode==31)||(TunMode==34)||(TunMode==44)||(TunMode==45))&&(CXD2837_DemodType==0)){
	sony_demod_GPIOSetConfig (pInteg->pDemod, 0, 1, 0);
	printf("[sony_integ_dvbt_Tune]--1,result=%d \n",result);
	if(TunerHV){
		//Set GPIO0 DVB-C LOW
		sony_demod_GPIOWrite (pInteg->pDemod, 0,1);
	}
	else
	{
	//Set GPIO0 DVB-C LOW
		sony_demod_GPIOWrite (pInteg->pDemod, 0,0);
	}
...
//#endif
printf("[sony_integ_dvbt_Tune]--4,result=%d \n",result);
        /* Tune the RF part */
        ...
        if (result != SONY_RESULT_OK) {
printf("[sony_integ_dvbt_Tune]--5,result=%d \n",result);
            SONY_TRACE_RETURN (result);
        }
                                           ...
                                           sony_dtv_system_t * pSystemTuned)
{
    ...
    if ((system == SONY_DTV_SYSTEM_DVBT) || (system == SONY_DTV_SYSTEM_ANY)) {
        ...
        if ((pInteg->pTunerTerrCable) && (pInteg->pTunerTerrCable->Tune)) {
			//#ifndef SONY_DISABLE_I2C_REPEATER
			if((TunMode==31)||(TunMode==34)||(TunMode==37)||(TunMode==44)||(TunMode==45))
			{
...
sony_result_t sony_integ_dvbt_t2_WaitTSLock (sony_integ_t * pInteg)
{
    ...
    printf("sony_integ_dvbt_t2_WaitTSLock\n");
...
if(TunMode==37)
{
        ...
        //        #endif
                ...
                result = sony_tuner_ascot2e_ReadGain (pInteg->pTunerTerrCable, &ifGain, &rfGain);
                if (result != SONY_RESULT_OK) {
                    SONY_TRACE_RETURN (result);
                }
...
sony_result_t sony_integ_dvbt2_monitor_SSI (sony_integ_t * pInteg, uint8_t * pSSI)
{
    ...
    result = sony_demod_dvbt2_monitor_ActivePLP (pInteg->pDemod, SONY_DVBT2_PLP_DATA, &plpInfo);
    if (result != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (result);
    }
    ...
    /* prel = prec - pref */
    prel = rfLevel - pRefdBm1000[plpInfo.constell][plpInfo.plpCr];
