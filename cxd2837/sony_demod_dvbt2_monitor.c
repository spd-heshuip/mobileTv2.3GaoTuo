/*------------------------------------------------------------------------------
  ...
  Last Updated  : $Date:: 2012-07-16 17:27:16 #$
  File Revision : $Revision:: 5810 $
------------------------------------------------------------------------------*/
...
sony_result_t sony_demod_dvbt2_monitor_SNR (sony_demod_t * pDemod, int32_t * pSNR)
{
...

    {
         ...
         */
        *pSNR = 10 * 10 * ((int32_t) sony_math_log10 (reg) - (int32_t) sony_math_log10 (24000 - reg));
        *pSNR += 34800;
    }
    SONY_TRACE_RETURN (result);
}
...
sony_result_t sony_demod_dvbt2_monitor_Quality (sony_demod_t * pDemod, uint8_t * pQuality)
{
    ...
    result = sony_demod_dvbt2_monitor_ActivePLP (pDemod, SONY_DVBT2_PLP_DATA, &plpInfo);
    if (result != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (result);
    }
