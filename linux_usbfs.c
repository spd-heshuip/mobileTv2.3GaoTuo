...
*/
	snprintf(path, PATH_MAX, "/dev/bus/usb/%03d/%03d", dev->bus_number,dev->device_address);
}
...
{
	const char *path = "/dev/bus/usb/001";//"/dev/bus/usb/001";
	const char *ret = NULL;
...
static int op_open(struct libusb_device_handle *handle)
{
	...
	//printf("op_open  0 %d  %s\n",handle->dev,filename);
	
	//hpriv->fd = open(/*filename*/"dev/usbdev1.2", O_RDWR);
